...
export const buildThursdayHyroxCircuit = (context: PlanContext): DailyPlan => {
  ...
  return base(
    ...
    plan('Practice race-specific stations under controlled fatigue.', easyWarmup('Tony'), [activity('HYROX circuit share', `${rounds} rounds: 0.5-mile run, then rotate stations: 500 meters SkiErg or RowErg, 50 feet sled push or sled pull, 8 burpee broad jumps, 12 wall balls`, 'RPE 7', '2 minutes between rounds', 'Keep the run controlled so stations remain strong.')], cooldown('Tony')),
    plan('Practice race-specific stations under controlled fatigue.', easyWarmup('Liz'), [activity('HYROX circuit share', `${rounds} rounds: 0.25-mile run/walk, then rotate stations: 250 meters SkiErg or RowErg, 30 feet light sled push or sled pull, 5 burpee broad jumps, 8 wall balls`, 'RPE 6', '2-3 minutes between rounds', 'Use planned breaks before form changes.')], cooldown('Liz')),
